...
from .models import Home, Category, Images
...
def listings(request):
    ...
    homes = Home.objects.filter(is_delete = False)
    ...
    return render(request=request, template_name='listings.html', context={"homes" : homes, 'categories':Category.objects.all()})
...
def single_listing(request, id):
    ...
    images = Images.objects.filter(home=home, is_delete=False)
    
    return render(request=request, template_name='single-listings.html', context={'home' : home, 'images' : images})